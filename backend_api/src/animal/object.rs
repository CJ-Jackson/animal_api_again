...
impl From<AnimalValidationError> for AnimalErrorObject {
    fn from(value: AnimalValidationError) -> Self {
        Self {
            species: value.species.err().map(|v| v.0.iter().map(|s| s.clone()).collect() ).unwrap_or_default(),
            description: value.description.err().map(|v| v.0.iter().map(|s| s.clone()).collect() ).unwrap_or_default(),
        }
    }
}